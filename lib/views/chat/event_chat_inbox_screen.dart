...
class EventChatInboxScreen extends StatefulWidget {
  final String? chatId;
  final String? title;
  final String? coverImage;
  final int? eventId;

  const EventChatInboxScreen({
    super.key,
    this.chatId,
    this.title,
    this.coverImage,
  ...
  @override
  State<EventChatInboxScreen> createState() => _EventChatInboxScreenState();
}

class _EventChatInboxScreenState extends State<EventChatInboxScreen>
  ...
  Future<void> _checkIfCreator() async {
    ...
    await eventController.getEventDetails(id: widget.eventId!);
    final event = eventController.eventDetails.value;
    if (event == null || _currentUserId == null) return;

    if (!mounted) return;
    setState(() {
      _isCreator = event.createdBy?.id == _currentUserId;
    });
  }
  ...
  Future<void> _loadInitialData() async {
    ...
    // Run creator check and chat history fetch in parallel
    final futures = <Future>[
      _checkIfCreator(),
      if (widget.chatId != null && widget.chatId!.isNotEmpty)
        _chatController.getChatHistory(chatId: widget.chatId!),
    ];
  ...
  Widget _buildMessageList() {
    ...
    return Obx(() {
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) => _EventMessageTile(
    ...
    return AppBar(
      ...
      title: Row(
        children: [
          ...
          Expanded(
            child: Text(
              widget.title?.isNotEmpty == true ? widget.title! : 'Event Chat',
  ...
  Widget _buildInputArea() {
    // Non-creators see a locked banner instead of the input
    if (!_isCreator) {
      return Container(
        ...
        child: Row(
          ...
          children: [
            ...
            SizedBox(width: 8),
            Text(
              'Only the event creator can send messages',
              style: TextStyle(
                color: Theme.of(context).colorScheme.onSurfaceVariant,
...
class _EventMessageTile extends StatefulWidget {
  ...
  @override
  State<_EventMessageTile> createState() => _EventMessageTileState();
}

class _EventMessageTileState extends State<_EventMessageTile>
